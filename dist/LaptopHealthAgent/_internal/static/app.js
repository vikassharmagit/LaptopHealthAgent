...
renderRecommendations = function(recs) {
  ...
  executableRecs.forEach((item) => {
    ...
    const categoryIcon = item.category === "storage" ? "💾" : item.category === "performance" ? "⚡" : "⚙️";
    
    row.innerHTML = `
      <div class="rec-content">
        <h3>${categoryIcon} ${escapeHtml(item.title)}</h3>
    ...
    btn.addEventListener("click", () => triggerAction(item.id, btn));
...
function renderRecommendations(recs) {
  ...
  executableRecs.forEach((item) => {
    ...
    const detailHtml = linkifyWindowsPaths(item.detail);
